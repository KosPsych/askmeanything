...
export async function update_after_edit(data:any) {
...
export async function getUsername(req : Request ,res : Response, next : Function) {
    const token = req.headers['x-observatory-auth']
    const options = {
        url: 'http://localhost:4004/get_user',
        method: 'GET',
        headers: {'x-observatory-auth': token}
    }
    axios(options).then(( res: Response) => {
        // @ts-ignore
        req.app.locals.username = res.data.username
        next()
    })
}